...
def build_internvl3_question(query: str, *, has_image: bool) -> str:
    """Format a user turn for InternVL ``model.chat``."""
    if has_image:
...
def __getattr__(name: str):
    if name in _INTERNVL_REEXPORTS:
        from procrustes import internvl_utils

        return getattr(internvl_utils, name)
